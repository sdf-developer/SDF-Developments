...
function injectComingSoonPopup() {
  ...
  style.textContent = `
    #cs-overlay {
      ...
      opacity: 0; animation: csIn 0.5s 0.6s cubic-bezier(0.16,1,0.3,1) forwards;
      pointer-events: none;
    }
    #cs-overlay.visible { pointer-events: all; }
  ...
  `;
  document.body.appendChild(overlay);
  // Activar pointer-events una vez que la animación de entrada ha terminado
  setTimeout(() => { if (overlay.parentNode) overlay.classList.add('visible'); }, 1200);
